...
from phi_core.control.policy import MANIFESTS, OUTPUT_SCHEMAS, CapabilityDenied, CapabilityPolicy
...
_RECORD_FIELDS = {
    ...
    AgentManifest: {"schema_version", "agent", "manifest_version", "purpose", "task_types", "accepted_input_classes", "data_class_ceiling", "output_schema", "allowed_tools", "network_domains", "allowed_providers", "allowed_models", "scope", "reads", "writes", "budget", "allowed_child_task_types", "max_depth", "max_children", "agent_id", "role", "permitted_senders", "permitted_recipients", "allowed_input_data_classes", "forbidden_input_data_classes", "forbidden_tools", "network_permissions", "child_agent_permissions", "expected_output_schema", "timeout_policy", "retry_policy", "acceptance_criteria"},
    ...
    ArtifactRecord: {"schema_version", "artifact_id", "session_id", "run_id", "producer_task_id", "scope", "type", "root", "rel_path", "sha256", "size_bytes", "state", "data_class", "retention_class", "parents", "gate_result_ids", "generation", "created_at", "promoted_at", "expires_at", "rejection_reason", "deleted_at", "delete_attempts", "delete_error", "hold"},
    HumanReviewRequest: {"schema_version", "request_id", "run_id", "session_id", "workflow_version", "task_id", "node", "reason_codes", "decision_version", "audit_version", "evidence_version", "required_role", "state", "created_at", "resolved_at", "superseded_by", "superseded_reason", "superseded_policy_version"},
    ...
    TraceEvent: {"schema_version", "event_id", "run_id", "seq", "session_id", "task_id", "parent_task_id", "depth", "attempt", "span_id", "phase", "direction", "parent_msg_id", "payload", "agent", "agent_version", "workflow_version", "prompt_version", "policy_version", "rule_version", "manifest_version", "provider", "model", "endpoint", "provider_request_id", "usage", "cost_usd", "latency_ms", "tool_requested", "tool_policy_decision", "tool_executed", "tool_result_status", "input_class", "output_class", "evidence_ids", "gate_ids", "review_ids", "artifact_ids", "outcome", "retry_category", "error_correlation_id", "status_text", "egress_digest", "gateway_decision", "fence", "prev_hash", "hash", "ts", "trace_id", "model_version", "prompt_template_version", "handoff_id", "sanitized_rationale", "alternatives_considered", "authorization_result", "failure_class", "error_code", "correction_number", "previous_state", "new_state", "agent_role", "attempt_id", "event_type", "input_artifact_refs", "output_artifact_refs", "tool_call_id", "decision", "policy_checks", "human_review_ref"},
...
def test_policy_has_the_exact_manifest_roles_and_web_search_research_boundary() -> None:
    assert set(MANIFESTS) == {
        "Lexicon", "Schema", "Instrument", "RegulationsExpert", "PHIMethodsExpert", "Judge", "Executor", "Scout", "Ledger", "Ledger.Compare", "Ledger.Aggregate", "Herald", "Herald.Abstract", "Herald.Sections", "Manager", "Operator", "Reviewer", "CorpusResearcher",
    ...
    assert web_enabled == {"RegulationsExpert", "PHIMethodsExpert", "Scout", "CorpusResearcher"}
    assert all(MANIFESTS[name].allowed_tools == {"web_search": 3} for name in web_enabled)


def test_output_schemas_reserves_study_knowledge_package_for_phase_5() -> None:
    """Wave 5/6-rename pre-reservation (docs/MASTER_ARCHITECTURE_V2.md section 28):
    Phase 5 builds the `StudyKnowledgePackage` record itself in `records.py`; this
    wave only registers the schema key in `control/policy.py` so Phase 5 does not
    need to touch this now-reserved file for that one line."""
