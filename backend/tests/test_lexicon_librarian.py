...
key, no Mongo, agents built directly with ``make_ctx``.
"""
...
def test_lexicon_indexes_every_row_even_when_llm_returns_fewer_gists(tmp_path):
    ...
    lex = _DropCountStubLexicon(make_ctx("Lexicon"), drop_count=4)
    ...
    missing_logs = [d for d in lex.ctx.trace.legacy_messages if d.phase == "lexicon.gist_missing"]
...
def test_missing_headers_logged_and_skipped(tmp_path):
    ...
    empty_logs = [d for d in lex.ctx.trace.legacy_messages if d.phase == "lexicon.empty:f1"]
...
def test_blank_name_rows_are_logged_as_one_aggregate_event(tmp_path):
    ...
    blank_logs = [d for d in lex.ctx.trace.legacy_messages if d.phase == "lexicon.blank_name"]
    ...
    parsed_logs = [d for d in lex.ctx.trace.legacy_messages if d.phase == "lexicon.parsed:f1"]
...
def test_answer_grounds_correction_only_in_that_columns_row(tmp_path):
    ...
    lex = _AnswerStubLexicon(make_ctx("Lexicon"))
    dict_files = [{"file_id": "f1", "original_name": "dictionary.csv",
                  "stored_path": str(path)}]
    asyncio.run(lex.run(dict_files=dict_files))
