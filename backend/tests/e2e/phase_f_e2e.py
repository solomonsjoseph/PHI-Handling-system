...
Steps:
  ...
  6. GET /preview -> confirm masked originals + bounded samples
  7. POST /human-review with actual_knowledge_ack=false -> 400
  8. POST /human-review with actual_knowledge_ack=true  -> 200
  9. Poll to `complete`
...
def main() -> int:
    ...
    # Phase D: preview endpoint
    r = requests.get(f"{BASE}/api/sessions/{sid}/preview?samples=5", timeout=TIMEOUT)
    r.raise_for_status()
    prev = r.json()
    file_summary = [(f["file_name"], len(f["samples"])) for f in prev["files"]]
    print("[6] preview files =", file_summary)
    raw_leaks = {"James Smith", "Mary Jones", "Peter Wong",
                 "415-555-1234", "415-555-9876", "415-555-5555",
                 "1975-03-15", "1982-07-22", "1990-11-01"}
    for f in prev["files"]:
        for s in f["samples"]:
            om = s.get("original_masked", "")
            assert om not in raw_leaks, f"[LEAK] preview returned raw PHI: {s}"
    print("[6] preview leak-check PASS")
    ...
    # Now resolve any human_review decisions to `drop` and submit with ack=true
    ...
    body["resolutions"] = [
        {"file_id": d["file_id"], "column": d["column"], "action": "drop"} for d in pending
    ]
    body["actual_knowledge_ack"] = True
    r = requests.post(f"{BASE}/api/sessions/{sid}/human-review", json=body, timeout=TIMEOUT)
    assert r.status_code == 200, r.text
    print("[8] submit with ack=True -> HTTP 200, status =", r.json().get("status"))
