...
def score_record(
    ...
    def _is_match(p: PredictedSpan, g: GoldSpan) -> bool:
        ...
        if entity_type_agnostic:
            return True
        return p.mapped_type == g.entity_type
...
def aggregate_record_scores(
    ...
    result = BenchmarkResult(tool_name=tool_name, gap_entity_types=set(gap_entity_types))
    ...
    result.coverable_span_count = agg.tp + agg.fn  # total non-gap gold spans
...
def print_report(result: BenchmarkResult, verbose: bool = False) -> None:
    ...
    print()
    print(f"AGGREGATE SCORES (coverable spans only):")
