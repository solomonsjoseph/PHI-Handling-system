#!/usr/bin/env python3
"""Prepare a SoT source pack from a PDF and row-1 dataset headers only."""
...
def _headers_via_repo(repo_root: Path, dataset: Path, *, run_dir: Path | None = None) -> list[str]:
    # The archived plugin optionally consulted scripts.extraction.header_store via
    # sys.path mutation. That store was not ported into the standalone package;
    # the SoT contract only needs row-1 headers, so the packaged reader is the
    # single source of truth and still falls back to direct parsing if imports fail.
    try:
        from phi_engine.sot.study_intake import read_headers_only
    except ImportError:
...
def _pdf_pages_via_repo(repo_root: Path, pdf: Path) -> list[dict[str, Any]]:
    try:
        from phi_engine.sot.study_intake import _read_form_pdf_pages
    except ImportError:
...
def build_source_pack(
    *,
    ...
    pdf = (repo_root / pdf).resolve() if not pdf.is_absolute() else pdf.resolve()
    dataset = (repo_root / dataset).resolve() if not dataset.is_absolute() else dataset.resolve()

    headers = _headers_via_repo(repo_root, dataset, run_dir=run_dir)
    ...
    renders: list[str] = []
    if render_dir:
        renders = _render_pdf(pdf, render_dir.resolve(), page_count=len(pages))
...
def main() -> int:
    ...
    print(f"headers: {len(pack['headers'])}")
