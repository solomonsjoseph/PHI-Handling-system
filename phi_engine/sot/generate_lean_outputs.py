...
4. Apply deterministic discrepancy/hold checks against the generated candidate.
5. After the candidate clears those checks, write the policy YAML + per-form schema (the
...
from phi_engine.audit.review_paths import is_sot_review_report_path, sot_review_report_path
from phi_engine.sot.extract_sources import build_source_pack
from phi_engine.sot.generate_pdf_aware_candidate import build_candidate
...
def generate_form(
    ...
    """Generate one form's SoT; construction artifacts go to audit, joined view to llm_source."""
    ...
    build_source_pack(
        ...
        dataset=dataset,
        out=source_pack,
        # The archived plugin rendered 600-DPI screenshots through Ghostscript for
        # visual review. The standalone runtime only needs PDF text/annotations +
        # row-1 headers for deterministic SoT enrichment, so render_dir is omitted
        # here to avoid making Ghostscript a pipeline dependency.
        render_dir=None,
        run_dir=run_dir,
    )
    candidate_data = build_candidate(repo_root, form, source_pack)
    candidate.parent.mkdir(parents=True, exist_ok=True)
    candidate.write_text(
        yaml.safe_dump(candidate_data, sort_keys=False, allow_unicode=False, width=120),
        encoding="utf-8",
    )

    # The archived generate_form() shelled out to check_lean_policy.py and, when
    # an original Indo-VAP anchored-gold policy happened to exist, to a missing
    # scripts/source_truth/diff_against_gold.py. Neither validator was part of the
    # standalone port: check_lean_policy.py is outside this package's contract, and
    # the gold-diff branch is dead for every new deployment because those original
    # data/SoT/<study>/<form>_policy.lean.yaml files are not present. The kept
    # safety gate is the live discrepancy detector below: discrepancy-bearing
    # candidates still route to a human-review report instead of being promoted.
            ...
            issues=[
                {
                    ...
                    "detail": (
                        "The candidate policy documents an un-reviewed Source Truth "
                        "discrepancy; see the review report for the recorded discrepancy kind."
                    ),
                }
            ],
            resolved_pdf=pdf,
            resolved_dataset=dataset,
            action_taken="candidate policy was flagged for human review",
            required_next_step=(
                "confirm the documented discrepancy, update the policy metadata if needed, "
                "then rerun SoT generation"
            ),
        )
        return review_path
