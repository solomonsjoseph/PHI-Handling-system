...
3. Calls ``phi_engine.sot.extract_sources.build_source_pack`` in-process; no
   plugin-relative path or subprocess wrapper is required in the standalone package.
...
Usage
-----
    python -m phi_engine.sot.study_intake \\
...
def _rejected_dataset_names(study_dir: Path) -> frozenset[str]:
    ...
    try:
        ...
        manifest_path = Path(config.study_config_path("_forms_manifest.yaml", study=study_dir.name))
        raw = yaml.safe_load(manifest_path.read_text(encoding="utf-8")) if manifest_path.is_file() else {}
        reject = raw.get("reject") if isinstance(raw, dict) else []
        if not isinstance(reject, list):
            return frozenset()
        actual = [path.name for path in (study_dir / "datasets").glob("*") if path.is_file()]
        return frozenset(
            name
            for name in actual
            for pattern in reject
            if isinstance(pattern, str) and (name == pattern or fnmatch.fnmatch(name, pattern))
        )
    except Exception:
        return frozenset()
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="python -m phi_engine.sot.study_intake",
        description=(
...
def main(argv: list[str] | None = None) -> int:
    ...
    except Exception as exc:  # noqa: BLE001 -- CLI reports the source-pack failure
        print(
            "error: source-pack extraction failed. Check that the PDF/dataset are readable: "
