...
- **Is not an orchestrator DAG node** — `phi_engine.pipeline.run.run_pipeline`'s classification step resolves the active rulebook through `resolve_rulebook` (honoring the study's `rule_refresh`: `pinned_only` by default → deterministic/offline, `online_preferred` → opt-in live AI-extract), which wraps the pinned `phi_review.refresh_jurisdiction_rules` primitive with the caching/drift-detection layer. This skill is the operator CLI over that same layer for out-of-band rulebook inspection; it does not emit a SkillResult marker and is not a publish phase.
