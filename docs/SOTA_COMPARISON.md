...
phi_engine's own pattern-detection surface (`benchmarks/phi_engine_adapter.py`), evaluated on this repo's synthetic multi-jurisdiction corpus (`corpus/`, seed 42, `harness/generate_corpus.py`), strict-protocol F1 by jurisdiction (`benchmarks/results/comparison_table.md`, generated 2026-07-07):
...
| in (56 / 112) | 0.5132 | 0.1014 | 0.1014 | 0.4203 (strict N/A) |
| eu (40 / 84) | 0.3333 | 0.0000 | 0.0000 | 0.5000 (strict N/A) |
| br (36 / 72) | 0.5806 | 0.0000 | 0.0000 | 0.5833 (strict N/A) |
| au (28 / 56) | 0.3663 | 0.0000 | 0.0000 | 0.4643 (strict N/A) |
| ug (28 / 56) | 0.3636 | 0.0000 | 0.0000 | 0.5000 (strict N/A) |

phi_engine end-to-end scrub system (`harness/run_phi_system.py`, `docs/JURISDICTION_EVIDENCE_REPORT_IN.md` / `_US.md`): redaction recall 99.17% (India, seed 42, 719 gold PHI cells, 6 unredacted -- all attributable to a disclosed 1/61 zero-jitter-offset property, not a detection miss) and 99.58% (USA, seed 42, 719 cells, 3 unredacted, same cause). Zero residual PHI findings in either published output tree.

## 3. Positioning argument: three axes, not free-text NER

**Do NOT read Section 2's numbers as "beats SOTA on free-text de-identification."** They do not, and the system is not designed to. Section 1's SOTA systems are transformer NER / LLM pipelines evaluated on *real, license-gated, narrative clinical text* (i2b2 2014, UK hospital records, UCSF notes). `phi_engine`'s detection surface (`phi_engine/security/phi_patterns.py`) is a **regex/pattern catalog with checksum validation** (Verhoeff for Aadhaar, format validation for Indian phone numbers), evaluated here on a **synthetic, generator-authored corpus** the system's own author also built. It has no chance of beating fine-tuned ClinicalBERT/RoBERTa or GPT-4-with-ten-shot-prompting on narrative free text, and this document does not claim otherwise.

Three axes where the surveyed literature above has no directly comparable system, and where this repo's evidence (Sections above, plus `docs/JURISDICTION_EVIDENCE_REPORT_IN.md`/`_US.md`) is genuinely load-bearing:

**(a) Fail-closed structured/tabular clinical-study pipeline with utility-preserving transforms.** None of the systems in Section 1 operate on CRF-style tabular clinical-study data with a fail-closed contract (unparseable value -> blank/quarantine, never silently published raw) plus utility-preserving transforms specifically: SANT interval-preserving per-subject date jitter (verified this session: 56/56 `VISITDAT`-`COLLDAT` intervals preserved exactly, 60/60 subjects' per-subject offset internally constant across all their own dates), HMAC domain-separated pseudonymization (238/238 measured tokens format-valid, 60/60 subjects' pseudonym linked identically across three CRF forms), and count-only audit ledgers. The Section-1 systems redact or replace free-text spans; none of them measurably preserve a clinical-study-specific invariant like inter-visit interval structure.
...
**(c) A multi-jurisdiction synthetic PHI benchmark with span-level gold + statutory authority citations, evidence-first from generation through publication.** i2b2 2014 is US-only, real-patient-derived, and DUA-gated (a genuine methodological strength for realism, but it structurally cannot be extended to India/EU/Brazil/Australia/Uganda without new DUAs per jurisdiction, and it is not freely reproducible). This repo's corpus (886 records / 2506 spans, 7 jurisdiction groups, `corpus/MANIFEST.json` sha256 `5b413b9df27985c961a9b4b688efd9706e194607f2d1bdfeb97eb7054c34e2be`, `validation_status: PASS`) is fully synthetic, seed-reproducible, and every span carries a statutory authority citation (`GoldSpan.authority`) traceable to `authorities/AUTHORITY_MATRIX.md`. No system surveyed in Section 1 offers this.

## 4. Known attacks on this positioning, and mitigations actually applied

**Circularity** (the system evaluated on its own synthetic corpus, generated by the same codebase). Mitigations applied and measured this session, not merely asserted: held-out-seed replication (Step 5.6 -- seed 1337 India run produced byte-for-byte identical `redaction_recall`, `per_subject_offset_all_constant`, and fail-closed match counts as seed 42, `benchmarks/results/phi-system-in-heldout/phi_system_result.json`); both scoring profiles reported for every benchmark run (strict + legacy, `benchmarks/results/comparison_table.md`); baselines (Presidio stock/tuned, spaCy) run on the identical corpus files as phi_engine, same `benchmarks.metrics` scoring code; every artifact hashed (sha256 recorded in every `phi_system_result.json`, `release_evidence.json`); this limitation is stated explicitly in both jurisdiction evidence reports. Third-party corpus validation (i2b2 2014, via DUA) remains a gated future step -- not claimed as done anywhere in this repo.

**Non-US fairness.** Stock Presidio has zero Aadhaar/ABHA/Indian-voter-ID/Indian-driving-license recognizers, so its recall on non-US jurisdictions is structurally low by construction, not by measurement error -- confirmed directly this session: strict F1 = 0.0000 for Presidio (stock and tuned identically) on `eu`, `br`, `au`, `ug`, and 0.1014 on `in` (`benchmarks/results/comparison_table.md`), against phi_engine's non-zero strict F1 on every one of those jurisdictions (0.3333-0.5806). `gap_detection_rate` is reported per tool per jurisdiction in every result JSON precisely so this structural-coverage gap is visible rather than laundered into a bare F1 number -- mirroring the pattern `.phi-build-status` already used for the single-jurisdiction 28.4%-gap figure cited before this session's corpus refresh.
...
- Not validated on real patient data. Every number in Section 2 and in `docs/JURISDICTION_EVIDENCE_REPORT_IN.md`/`_US.md` is synthetic-corpus-scoped.
- Not clinician-reviewed or counsel-reviewed (tracked as open blockers in `.phi-build-status`).
- Not "fail-proof" or unscoped "100% accurate" anywhere, per repo Claim Discipline (CLAUDE.md Truth Protocol). The bounded equivalents used throughout this repo's reports: "redacted M/M (100%) of gold-annotated PHI cells on this corpus" (only printed when actually measured -- see the 99.17%/99.58%, not 100%, numbers actually measured this session) and "outperforms all N evaluated baselines on this benchmark" (scoped to the named baselines and named benchmark, never generalized).
