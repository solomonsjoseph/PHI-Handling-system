...
Mitigation (current, limited): `harness/mia_framework.py` runs a deterministic smoke test -- a logistic-regression classifier trained on record-level structural features (text length, span counts, etc.) against an arbitrary index-parity label, evaluated via 5-fold cross-validated AUC. This is NOT a shadow-model MIA per Shokri et al. (2017) or the Nature Scientific Reports 2024 methodology: it trains no shadow/target model pair and does not attack an actual PHI-detection model's outputs. It only checks whether structural corpus features are trivially separable, as a fast pre-release sanity check. A true shadow-model MIA against a deployed detector is planned but not implemented (see `docs/KNOWN_LIMITATIONS.md`). The smoke test must be run against each corpus release and its output included in the release documentation, with this scope limitation stated alongside the result.
