...
- `run_decision_gates` (`0004-artifact-registry.md`'s sibling ADR for the decision gate, recorded when Phase 4's `WorkflowRun`/`decision_version` wiring lands under `0001-workflow-engine.md`) does not itself consume `EvidenceClaim` records yet; Statute/Praxis output still reaches Judge as a plain dict. Binding decision-level evidence citations to a specific `EvidenceClaim` id is Phase 5+ scope, tracked alongside `F-EVID-001`.
