...
RegulationsExpert's HIPAA-rule and adjacent-regime lookups, PHIMethodsExpert's method research, Scout's citation-bearing findings, and `CorpusResearcher`'s dataset-repository research each now correlate their reported sources to the response's actual tool-call citations and build `EvidenceClaim`/`EvidenceSource` records through this module. When a claim does not reach `VERIFIED`, each agent falls back to its own deterministic, non-LLM default (RegulationsExpert's baked-in `_pack_fallback`, PHIMethodsExpert's `_fallback(category)`) rather than retrying the same ungrounded call or degrading the result silently.
...
- `run_decision_gates` (`0004-artifact-registry.md`'s sibling ADR for the decision gate, recorded when Phase 4's `WorkflowRun`/`decision_version` wiring lands under `0001-workflow-engine.md`) does not itself consume `EvidenceClaim` records yet; RegulationsExpert/PHIMethodsExpert output still reaches Judge as a plain dict. Binding decision-level evidence citations to a specific `EvidenceClaim` id is Phase 5+ scope.
