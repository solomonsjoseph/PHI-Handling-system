...
- Acceptance tests: `backend/tests/test_control_superorchestrator.py`, `backend/tests/test_certification_invalidation.py`, `backend/tests/test_manager.py`, `backend/tests/test_production_readiness.py`, `backend/tests/test_control_bounds.py`, `backend/tests/test_architecture_boundaries.py`
- Status: open
- Disposition: Phase 5 lands `SuperOrchestrator`, the D9 exclusive-authority class (fenced node transitions, run-wide/parallel/fanout/depth/budget-widen-checked child delegation, review request/consume, acceptance) and routes `session_handle`/`session_human_review`/`session_cancel`/`session_delete`'s durable root and cancellation work through it. `Manager.escalate_to_human_review` is deleted (D10); every former caller, plus `run_pipeline`'s decide-loop escalation, now calls a shared `orchestrator._escalate_to_human_review` helper that persists the session document and then calls `SuperOrchestrator.request_human_review` at the exact D9 node each site models. `test_manager_holds_no_workflow_authority` and `test_only_the_artifact_service_writes_the_publication_pointer` (static AST scans) and `test_concurrent_child_creation_cannot_exceed_parent_ancestor_or_run_budgets` pass against real code.
- Residual risk: `session_intake`, corpus paths, warmup, and startup recovery remain direct. Step 5 (Ledger/Herald as durable children) is open. Step 7 is partial: `create_child_work` enforces 5 of 18 D5 bounds; the remaining 11 need accumulator infrastructure in the gateway and `ArtifactService` that does not exist yet. Step 8 surfaced a scope discovery: `control/activation.py::ActivationFactory.activate` -- the mechanism EVERY individual agent activation across the pipeline uses, not just Ledger/Herald -- is a real, currently-necessary `TaskService.enqueue` caller outside `SuperOrchestrator`, documented in its own module docstring as an interim exception pending this exact migration. `test_no_module_outside_the_super_orchestrator_calls_task_service_enqueue` and `test_every_entry_path_submits_a_command` cannot pass until that migration (materially larger than step 5's two-agent framing: all ~20 agent activations) and the remaining step 2 routes land.
...
- Status: open
- Disposition: `REVIEWER_PRINCIPALS`-backed authorization now gates `session_human_review` (D13 step 1) and boots insecure without it outside dev. Comment-mode resolution never auto-applies regardless of confidence (D13 step 6): every model interpretation of free text now requires a separate, explicit reviewer confirmation. `auditor_escalation_reason` now blocks a high-confidence `issues` verdict unconditionally and rejects an audit naming an unknown or hash-mismatched artifact (plan step 2, partial: evidence-sufficiency and deterministic-gate-result inputs to the same gate are not yet wired -- no established data path threads `EvidenceClaim`/`GateResult` objects into Auditor's context). `Judge.run` now returns a typed `JudgeProposal` (pydantic), validated immediately after the LLM call rather than trusted as a bare dict (plan step 1); `validate_decisions` still owns vocabulary correctness unchanged. `HumanReviewSubmit.client_event_id` is required (D13 step 4); a submission is checked for `(request_id, client_event_id)` idempotency against the durable `HumanReviewRequest`/`HumanReviewEvent` records when one exists for the run (additive: a pre-D9-migration session with no durable request falls through unchanged, and a resubmission that arrives *after* the request has already resolved gets no idempotency protection -- caught instead by the pre-existing session-state-conflict 409). A submission that resolves the pipeline persists its event through `SuperOrchestrator.consume_review_event`, which marks the request resolved; a still-awaiting submission persists its event directly. `decision_version` itself is owned end-to-end by `run_decision_gates` (D11, verified in `test_control_decisions.py`), but `session_human_review`'s own gate call still runs with no `store`, so `decision_version` stays 0 on that path and the D13 step 8 delivery-gate match on `(principal, file_id, decision_version)` is not yet implemented; today's check remains "any download ever occurred." `control/review.py::HumanReviewService` (D13's typed service class) and the `review_resume` outbox-durable-drain architecture do not exist -- `session_human_review` is still one large synchronous route with authorize/idempotency logic inlined, not extracted behind that class. `audit_version`-required-when-`confirm_auditor_confidence` (D13 step 7) and Operator/Reviewer's source-vs-export value comparison (plan step 6) are not started.
