...
- Acceptance tests: `backend/tests/test_control_superorchestrator.py`, `backend/tests/test_certification_invalidation.py`, `backend/tests/test_manager.py`, `backend/tests/test_production_readiness.py`, `backend/tests/test_control_bounds.py`, planned `backend/tests/test_architecture_boundaries.py`
- Status: open
...
- Residual risk: `session_intake`, corpus paths, warmup, and startup recovery remain direct. Step 5 (Ledger/Herald as durable children) and step 8 (boundary tests) remain open. Step 7 is partial: `create_child_work` enforces 5 of 18 D5 bounds (depth, fanout, run-wide task count, run-wide and per-parent parallelism, budget-widen); the remaining 11 (wall-clock, token/cost/tool-call/artifact-byte running totals) need accumulator infrastructure in the gateway and `ArtifactService` that does not exist yet, tracked separately from this finding.
