...
- `test_comment_resolution_never_auto_applies_regardless_of_confidence`, `test_reviewer_principals_parses_name_role_pairs_and_drops_unknown_roles`, `test_reviewer_role_grants_dev_lead_reviewer_only_when_unset_and_only_to_dev`, `test_reviewer_role_respects_configured_principals_over_the_dev_fallback`, `test_auditor_escalation_reason_blocks_a_high_confidence_issues_verdict`, `test_auditor_escalation_reason_catches_unknown_or_mismatched_artifact_identity`, `test_high_confidence_auditor_issue_blocks_completion`, `test_judge_typed_proposal.py` (5 tests), `test_a_successful_submission_persists_its_event_and_resolves_the_durable_request`, `test_client_event_id_is_idempotent_while_the_request_stays_open`. `ruff check backend`; `cd backend && DATA_DIR=<fresh tmp dir> pytest tests -q -rs`.

### 7. Exact results, including every failure and every skip with its reason.

- Ruff: clean throughout. Every touched/new test file passed individually and in per-file groups: `test_certification_invalidation.py` 17/17, `test_manager_checkpoints.py` 13/13, `test_control_decisions.py` 18/18, `test_judge_typed_proposal.py` 5/5, `test_production_readiness.py` 16/16, `test_abuse_cases.py` + `test_human_review_invariant.py` + `test_human_review_resume_execution.py` 40/40 combined. A full-suite run (`pytest tests -q -rs`, all ~790 tests) could not be completed during the `client_event_id`/idempotency slice: the host is a Slurm-managed shared compute node, and repeated attempts stalled with pytest processes observed in kernel D-state (uninterruptible disk I/O wait, not CPU-bound) while `uptime` showed load average 3.3-5.6 against 4 cores and other users' concurrent heavy jobs were visible in `ps aux`. Grep-verified no call site of `Auditor()`, `Judge()`, `HumanReviewSubmit()`, or `session_human_review()` exists outside the files already individually verified, so this is recorded as an environmental verification gap, not a passing claim for the untested remainder.
