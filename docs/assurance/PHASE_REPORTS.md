...
- `cancel_subtree` (part of step 7's "await or fence children") is not yet meaningfully callable from `session_delete`: no production route enqueues pipeline work through `TaskService` (steps 4/5), so no `WorkItem` tree exists to cancel. The tombstone-then-erase ordering already implemented is what actually prevents resurrection today; recursive cancellation becomes real once steps 4/5 land.
- Phase 4 steps 4, 5, and the remainder of 8 and 10 remain open: `_run_tail` still duplicates the initial pipeline path (the single largest remaining piece: `session_handle`/`session_human_review` must enqueue through `TaskService` and resume via `control/workflow.py`'s node table rather than run pipeline logic inline); recursive cancellation across every pipeline exit path; the 900-second orphan sweep is not yet replaced by lease reconciliation (nothing yet populates `work_items` for a live pipeline run, so there is nothing for `reconcile_leases` to reconcile); and the boundary kill tests (`test_kill_between_outbox_boundaries`) are not written. See `F-DUR-001`, `F-ORCH-001`, and the Phase 4 row in `docs/assurance/RISK_REGISTER.md`.
