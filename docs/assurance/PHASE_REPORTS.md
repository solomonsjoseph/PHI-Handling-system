...
- `cancel_subtree` (part of step 7's "await or fence children") is not yet meaningfully callable from `session_delete`: no production route enqueues pipeline work through `TaskService`, so no `WorkItem` tree exists to cancel. The tombstone-then-erase ordering already implemented is what actually prevents resurrection today; recursive cancellation becomes real once `session_handle`/`session_human_review` enqueue through `TaskService`.
- Phase 4 steps 5 (Scout's asyncio-level leak is closed; its `TaskService`-lifecycle management is not), the remainder of step 8 (nothing yet populates `work_items` for a live pipeline run, so the 900-second orphan sweep cannot yet be replaced by lease reconciliation), and step 10's boundary kill tests remain open. `session_handle` and `session_human_review` still run pipeline work as a bare per-request `asyncio.create_task`, not a `TaskService`-enqueued, `Worker`-claimed task; `control/worker.py`'s `OUTBOX_HANDLERS` registry is still empty. See `F-DUR-001`, `F-ORCH-001`, and the Phase 4 row in `docs/assurance/RISK_REGISTER.md`.