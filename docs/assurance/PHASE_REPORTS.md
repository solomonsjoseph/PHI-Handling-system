...
## Phase 5 (complete): Super Orchestrator and bounded delegation

All nine plan steps are complete. Steps 1, 3, 6, and 9 landed and were verified at the checkpoint below; steps 2, 4, 5, 7, and 8 (routing every provider/workflow-starting entry path through `SuperOrchestrator`, deleting `Manager.escalate_to_human_review`, converting Ledger/Herald to durable child work, enforcing all 17 relevant D5 bounds, and the delegation boundary tests) landed in the closing pass recorded at the end of section 8 below, alongside a real pre-existing gap that pass surfaced and fixed: no synchronous-pipeline agent activation ever completed its own `WorkItem`.
...
- Closing pass: steps 2, 4, 5, 7, and 8 are now complete, and Phase 5 is closed. Step 2: `corpus_study_research` and `settings_warmup`/`_run_warmup` (shared with `_warmup_scheduler_loop`) each call `start_run` (`run_type="maintenance"`/`"warmup"`) before their pre-existing `ActivationFactory`-issued grants; `corpus_study_run` was confirmed to already delegate to `session_handle` directly; `session_intake` and `corpus_study_generate` do no provider/workflow work and are correctly out of scope by the plan's own qualifier; the dead `_CORPUS_STUDY_TASKS` map is deleted. Step 5: Ledger's Compare/Aggregate and Herald's Abstract/Sections are `create_child_work` children of their parent's task (the `Ledger`/`Herald` manifests gained non-empty `allowed_child_task_types`), with `SuperOrchestrator.accept_result` as the sole authority accepting each child's material result. Converting them surfaced a real, pre-existing gap this pass also fixed: `ActivationFactory.activate` claims a task and hands back a context but nothing ever transitioned it out of `leased`, so `create_child_work`'s live-task accounting -- the first real caller of `MAX_PARALLEL_TASKS_PER_RUN`/`MAX_TASKS_PER_RUN` against real accumulated state -- would have refused in any real run, not only a test. `Agent.__init_subclass__` now wraps every leaf subclass's `run()` to complete (or fail) its own task against a new `AgentContext.tasks` (`TaskCompleter` protocol, `StoreTaskCompleter` implementation); Praxis's `method_for()`, the one calling convention that bypasses `run()`, completes explicitly at its two call sites. Three tests carried fake agent doubles that discarded their `ctx` and never completed it; updated to match real `Agent` behavior. Step 7: the remaining 11 D5 bounds are enforced -- 4 already-grant-scoped per-task ceilings (wall/tokens/cost/tool-calls) `ProviderGateway.complete` now checks explicitly, 2 new byte-size ceilings (`MAX_INPUT_BYTES`/`MAX_OUTPUT_BYTES`), and 5 run-level aggregates via a new `control/runs.py` accumulator (`check_run_budget`/`record_run_usage`) populating `WorkflowRun.usage`/`.budget` fields that existed but nothing used. A new `BudgetExceeded` (`CapabilityDenied` subclass) distinguishes a resource-ceiling refusal from an identity/scope/data-class denial; every refusal, enqueue-time and gateway-time alike, is recorded as `TraceEvent(outcome="budget_exceeded")`, satisfying the plan's per-bound requirement literally via `StoreTraceWriter`'s existing `event_seq` CAS-allocation (duplicated inline in `gateway.py` to avoid a `context.py`<->`gateway.py` import cycle). All 17 bounds have a dedicated test plus an env-var-override test in `test_control_bounds.py`, including the 6 already-landed enqueue-time ones retrofitted with the `TraceEvent` assertion. Step 8: `test_every_entry_path_submits_a_command` and `test_no_module_outside_the_super_orchestrator_calls_task_service_enqueue` are now written and pass, each documenting its one narrow, intentional exception (`session_intake`/`corpus_study_generate`'s no-provider-work scope; `ActivationFactory.activate`'s interim `TaskService.enqueue` status, pending the separate, materially larger ~20-activation migration this phase did not undertake) rather than a fictional clean state. `_startup_maintenance`'s orphan-run reconciliation still marks a stuck legacy `sessions` document `failed` directly without also transitioning its `WorkflowRun`; recorded as residual risk in `F-ORCH-001`, not fixed here (low-value, low-frequency boot-time recovery path). Backend suite: 813 passed, 8 skipped, ruff clean, across five commits.

## Phase 6 (complete): evidence and human review
...
- `test_comment_resolution_never_auto_applies_regardless_of_confidence`, `test_reviewer_principals_parses_name_role_pairs_and_drops_unknown_roles`, `test_reviewer_role_grants_dev_lead_reviewer_only_when_unset_and_only_to_dev`, `test_reviewer_role_respects_configured_principals_over_the_dev_fallback`, `test_auditor_escalation_reason_blocks_a_high_confidence_issues_verdict`, `test_auditor_escalation_reason_catches_unknown_or_mismatched_artifact_identity`, `test_high_confidence_auditor_issue_blocks_completion`, `test_judge_typed_proposal.py` (5 tests), `test_a_successful_submission_persists_its_event_and_resolves_the_durable_request`, `test_client_event_id_is_idempotent_while_the_request_stays_open`, `test_cap_age_90_wrong_value_passes_shape_but_fails_source_comparison`, `test_year_only_wrong_value_fails_source_comparison`, `test_zip3_truncate_wrong_value_fails_source_comparison`, `test_pseudonymize_inconsistent_mapping_fails_source_comparison`, `test_cap_age_90_without_source_still_passes_on_shape_alone`. `ruff check backend`; `cd backend && DATA_DIR=<fresh tmp dir> pytest tests -q -rs`.

### 7. Exact results, including every failure and every skip with its reason.

- Ruff: clean throughout, repo-wide. Every touched/new test file passed individually and in per-file groups: `test_certification_invalidation.py` 17/17, `test_manager_checkpoints.py` 13/13, `test_control_decisions.py` 18/18, `test_judge_typed_proposal.py` 5/5, `test_production_readiness.py` 16/16, `test_abuse_cases.py` + `test_human_review_invariant.py` + `test_human_review_resume_execution.py` 40/40 combined, `test_operator.py` 50/50 (45 pre-existing + 5 new), `test_full_path.py` 1/1 (the real `orchestrator.py` call site exercising the new source-comparison path against Operator's actual production wiring). A full `pytest tests -q -rs` (all ~790 tests) completed once during this slice: 780 passed, 8 skipped, 2 failed. Both failures (`test_production_boot_refuses_without_api_tokens`, `test_dev_boot_starts_with_no_configuration`) were `subprocess.TimeoutExpired` after 60s spawning a fresh Python interpreter under host load 5.4 on 4 cores (Slurm-managed shared node); both passed individually, 16/16, once re-run in isolation at load 3.9. Neither failure touches Operator or any file changed this slice.

### 8. Remaining risk and deferred work, cross-referenced to `docs/assurance/RISK_REGISTER.md`.

- Step 1 is done: `Judge.run` returns a typed `JudgeProposal` (pydantic), validated immediately after the LLM call; `validate_decisions` (D11's first gate) still separately owns vocabulary correctness.
- Step 2 is partial: see `F-HITL-001` for the exact evidence-sufficiency/deterministic-gate-result gap.
- Steps 3-5 are partial: `client_event_id` is required and idempotency-checked against the durable `HumanReviewRequest`/`HumanReviewEvent` records when one exists; a resolving submission persists its event through `SuperOrchestrator.consume_review_event`. `control/review.py::HumanReviewService` itself and the `review_resume` outbox-durable-drain architecture (no handler registered in `worker.py::OUTBOX_HANDLERS` yet) do not exist -- `session_human_review` remains one large synchronous route with this logic inlined, not extracted behind a service class, and a resubmission after the request has already resolved gets no idempotency protection (caught instead by the pre-existing session-state-conflict 409). `audit_version`-required-when-`confirm_auditor_confidence` (step 7 of D13) is not implemented.
- Step 6 is done for the plan's concrete target, `operator._verify_record`: for `cap_age_90`, `year_only`, `zip3_truncate`, and `pseudonymize`, a value comparison against the recomputed source transform (or, for `pseudonymize`, a consistency/injectivity check on the source-to-written mapping, since Operator has no registry salt) now runs whenever `stored_path` is available, catching a well-shaped but wrong value the pre-existing shape check alone could not. Absent a `stored_path`, verification falls back unchanged to the pre-existing shape-only check per the plan's "keep every existing check." `Reviewer` is unchanged: its docstring already establishes that re-deriving Operator's per-cell checks "would just be Operator running twice," so the plan's "Strengthen Operator and Reviewer" is satisfied by the `operator._verify_record` change alone, matching the plan's own concrete instruction.
...
- Step 8 (frontend `whoami()`/reviewer-identity plumbing) is not started; the frontend's `SessionDetail.jsx` submit call was updated only to the minimum needed to stay functional against the new required `client_event_id` field and the removed `reviewer` field.
- Closing pass: steps 3-5, 7, and 8 are now complete, and Phase 6 is closed. Step 3/4: `audit_version` is required whenever `confirm_auditor_confidence` is true and must match the open `HumanReviewRequest`'s own value (409 on mismatch, minted by the orchestrator as a content hash of Auditor's verdict at the escalation call site). `SuperOrchestrator.supersede_human_review` closes an open request without a review event; `request_human_review` calls it automatically whenever a new escalation targets a run that already has one open, so at most one open request per run_id always holds -- the exact gap D13 names ("a rerun of Auditor... cannot clear an older request; only supersede closes it"). `control/review.py::HumanReviewService` and the `review_resume` `OutboxEntry`/`drain_outbox` relay were deliberately not built as literally specified: `session_human_review` already ends by calling `SuperOrchestrator.start_run(..., root_task_type="pipeline_resume")`, which creates a durable `WorkItem` claimed by the pre-existing `Worker`/`TASK_HANDLERS["pipeline_resume"]` claim-and-lease loop -- the same durable-resume machinery `pipeline_run` (fresh sessions) already uses. A parallel outbox relay would duplicate that durability with no functional gain; see `F-HITL-001` for the full reasoning, including why D13 step 2's fence/`lease_owner` check has no target in this architecture (no `WorkItem` exists yet at review-submission time). Step 7: `decision_version` is real everywhere D13 needs it -- `_escalate_to_human_review`'s previously-hardcoded `0` now looks up the run's CAS-incremented value from `workflow_runs`, and `session_human_review`'s own gate call passes `store=control_store` whenever a `WorkflowRun` exists for the run. `dataset_file_downloads` entries now carry `decision_version`, and the actual-knowledge gate checks each resolved column's own file against `(principal, file_id, decision_version)`, replacing the old session-wide "any download ever occurred" check. Step 8: `SessionDetail.jsx` and `Wizard.jsx` replaced the operator-typed, localStorage-persisted "reviewer" field (never actually sent to the backend) with a read-only identity from `GET /api/auth/whoami`, plus a confidence-only confirmation control rendering Auditor's verdict/issues/confidence-as-telemetry and posting `confirm_auditor_confidence` with the open request's `audit_version`. Backend: 814 passed, 8 skipped, ruff clean. Frontend: fresh `npm ci` (node_modules had never been installed in this environment), 6/6 tests pass, production build compiles clean. Fixed in passing: a PHI-scrub gap where the escalation path's `audit` session-doc field (distinct from the post-completion `agent_audit` field) bypassed `scrub_nested`; a frontend test-infra bug where `jest.mock` factory-baked implementations were silently stripped by `react-scripts`' `resetMocks: true`; a pre-existing jsdom `crypto.randomUUID` gap that already failed on the untouched baseline.
