...
Seed: 42
Method: Truth Protocol (CLAUDE.md). Every number below is read directly from a committed artifact JSON; none is hand-typed. Reproduction commands are given for every claim. **Current scope is USA-only** (HIPAA Safe Harbor / LDS corpus and `PaperDemoUS`). Non-USA jurisdiction generators, rulebooks, and evidence reports (including the former India report) have been removed from the active tree and are not live artifacts.
...
| `phi_engine/config/PaperDemoUS/phi_scrub.yaml` sha256 | `b60de310e6ec3b18e147887a77afd207598edd34551b7a685dc23c1dfa792b5c` -- **byte-identical** to `phi_engine/config/_defaults/phi_scrub.yaml` (`b60de310...`, same hash). No per-study addition was needed: `SSN`, `MRN`, `PHONE_NUM`, `EMAIL` all matched the packaged `drop_fields` defaults directly (verified via `PHIScrubConfig.field_is_drop()` before writing the generator). |
| Run id | `20260707T145528Z` |
| n_subjects | 60 |

## 2. Corpus provenance

Canonical corpus: `corpus/MANIFEST.json` sha256 `5b413b9df27985c961a9b4b688efd9706e194607f2d1bdfeb97eb7054c34e2be`, `validation_status: PASS`. US (`us/`) totals: 550 records, 1314 gold spans across 10 generator groups (`hipaa_safe_harbor` 180/302, `hipaa_quasi_identifiers` 50/150, `hipaa_lds` 40/130, `hipaa_reid_codes` 40/40, `hipaa_fundraising` 40/265, `hipaa_verification` 40/195, `hipaa_biometric` 40/48, `hipaa_device` 40/64, `hipaa_fax` 40/68, `hipaa_vehicle` 40/52). Non-USA jurisdiction folders formerly present under `corpus/` are out of current scope.

## 3. New clinical-study tabular corpus

`generators/study_tabular.py::USStudyTabularGenerator` -- authority citation 45 CFR 164.514(b)(2) (HIPAA Safe Harbor). Three-form CRF shape (`1A_Screening.jsonl`, `2_Demographics.jsonl`, `3_Labs.jsonl`), shared column contract (`SUBJID`, `IC_SCRNNUM`, `VISITDAT`/`COLLDAT`/`TBTXDT`, `IS_BIRTHDAT`, `AGE`, `SEX`, `WEIGHT`, `CBC_HGB`), US-specific identifier columns `SSN`, `MRN`, `PHONE_NUM`, `EMAIL` (all four bind to packaged `drop_fields` defaults with no study-specific addition). Deliberate fail-closed edge cases at fixed subject indices (0: AGE=92; 1,2: VISITDAT="INVALID-DATE"; 3,4: Labs SUBJID="").
...
- `classifier_path: "fallback"` (`llm.provider: none`, GR-1 held, no cloud LLM).
- `header_classification_agreement: 0.2143` (the classifier's coarse drop/keep vocabulary and value-shape-oriented regex catalog; the underlying `phi_gate`/`presidio_gate` fallback scans header **names**, not jurisdiction-specific value patterns).
...
`redaction_recall = 716 / 719 = 0.9958` (99.58%, measured). `dates.shifted: 171 / 174` -- 3 date cells (1 subject x 3 date columns) whose per-subject SANT offset computed to exactly zero this seed (disclosed ~1/61-probability property of the SANT jitter algorithm: integer day offset drawn uniformly from `[-30, +30]` excluding 0 would be 60 outcomes; including the measured zero-offset event yields the ~1/61 rate). All 3 residual cells are explained by this mechanism; none is an unexplained finding.
...
Synthetic-only, single-cohort-per-seed, circularity of the phi_engine-vs-own-corpus benchmark, no clinician/counsel review, free-text NER explicitly out of scope. US held-out-seed anti-tuning replication was not independently re-run in this evidence pass (seed 42 only).
...
```
(Run under the repo's `.venv` -- Python 3.10+ required; `requires-python = ">=3.10"` in `pyproject.toml`.)
